	...
	public function index() {   
		...
		if((float)$opencartversion<1.51){
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {			
				$module=array();
				$i=0;
				if(isset($this->request->post['fbconnect_module'])){
					foreach($this->request->post['fbconnect_module'] as $k=>$v){
				...
				$this->request->post['fbconnect_module']=implode(',',$module);
			}
		}
		...
		if (isset($this->error['code'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/fbconnect', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/fbconnect', 'token=' . $this->session->data['token'], 'SSL');
		...
		if($opencartversion<1.51){
			...
			$toarray=$obj_get='';
			if(isset($this->request->post['fbconnect_module'])){
				$toarray=$this->request->post['fbconnect_module'];
				$obj_get='post';
			}
 			elseif ($this->config->get('fbconnect_module')!='') { 
				$toarray=$this->config->get('fbconnect_module');
				$obj_get='config';
			}
			if($toarray!=',' && $obj_get!=''){
			...
			if (isset($this->request->post['fbconnect_module'])) {
				$this->data['modules'] = $this->request->post['fbconnect_module'];
			} elseif ($this->config->get('fbconnect_module')) { 
				$this->data['modules'] = $this->config->get('fbconnect_module');
			}		
		}
