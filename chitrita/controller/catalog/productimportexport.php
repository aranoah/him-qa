...
class ControllerCatalogProductImportExport extends Controller {
	private $error = array(); 
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . $url, 'SSL'));
    	}
	...
	public function bulk() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateDelete())) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
		
		$this->data['bulk_action'] = $this->url->link('catalog/productimportexport/bulk', 'token=' . $this->session->data['token'] . $url, 'SSL');
  	...
  	private function getList() {				
		...
		$this->data['show50'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&pre_page=50' . $url, 'SSL');
		$this->data['show200'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&pre_page=200' . $url, 'SSL');
		$this->data['show500'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&pre_page=500' . $url, 'SSL');
		$this->data['show1000'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&pre_page=1000' . $url, 'SSL');
		...
		$this->data['export_select'] = $this->url->link('catalog/productimportexport/export', 'token=' . $this->session->data['token']. $url, 'SSL');
		$this->data['bulk'] = $this->url->link('catalog/productimportexport/bulk', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['insert'] = $this->url->link('catalog/productimportexport/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['copy'] = $this->url->link('catalog/productimportexport/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');	
		$this->data['delete'] = $this->url->link('catalog/productimportexport/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/productimportexport/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$this->data['sort_model'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, 'SSL');
		$this->data['sort_price'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, 'SSL');
		$this->data['sort_quantity'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
		$this->data['sort_order'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('catalog/productimportexport/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/productimportexport/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('catalog/productimportexport', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		/*if (isset($this->request->post['product_tag'])) {
			...
			$this->data['product_tag'] = $this->model_catalog_productimportexport->getProductTags($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_attribute'])) {
			...
			$this->data['product_attributes'] = $this->model_catalog_productimportexport->getProductAttributes($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_option'])) {
			...
			$product_options = $this->model_catalog_productimportexport->getProductOptions($this->request->get['product_id']);			
		} else {
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_catalog_productimportexport->getProductImages($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_related'])) {
			...
			$products = $this->model_catalog_productimportexport->getProductRelated($this->request->get['product_id']);
		} else {
			$products = array();
		}
		...
		foreach ($products as $product_id) {
			$related_info = $this->model_catalog_productimportexport->getProduct($product_id);
		...
		if (isset($this->request->post['product_reward'])) {
			...
			$this->data['product_reward'] = $this->model_catalog_productimportexport->getProductRewards($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_layout'])) {
			...
			$this->data['product_layout'] = $this->model_catalog_productimportexport->getProductLayouts($this->request->get['product_id']);
		} else {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				$product_options = $this->model_catalog_productimportexport->getProductOptions($result['product_id']);	
